...
exports.createGrocery = async (req, res) => {
  try {
    const grocery = await models.Grocery.create(req.body);
...
exports.getGroceryById = async (req, res) => {
  try {
    const grocery = await models.Grocery.findByPk(req.params.id);
    if (grocery) {
...
exports.updateGrocery = async (req, res) => {
  try {
    const [updated] = await models.Grocery.update(req.body, {
      where: { id: req.params.id },
    });
    if (updated) {
      const updatedGrocery = await models.Grocery.findByPk(req.params.id);
...
exports.deleteGrocery = async (req, res) => {
  try {
    const deleted = await models.Grocery.destroy({
      where: { id: req.params.id },
    });
    if (deleted) {
      res.status(204).send();
